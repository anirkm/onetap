...
exports.run = async (client, message, args) => {
        ...
        if (frCount != 0) {
          ...
          for (var friend of frData) {
            ...
            if (frUser) {
              try {
                console.log();
                await authorChannel.permissionOverwrites.edit(frUser, {
                  CONNECT: true,
                  SPEAK: true,
                  VIEW_CHANNEL: true,
                });
  ...
  async function listFriends(client, message) {
    ...
    await message.channel
      .send({ embeds: [frList], components: [row] })
      .then((msg) => {
        client.on("interactionCreate", async (button) => {
          ...
          switch (button.customId) {
            case "left_frlist":
              if (page === 1) return button.update({});
              page--;
              x = x - 5;
              frList.fields = [];
              try {
              ...
              frList.setFooter(`11PM Friends - Page: ${page}/${maxpages}`);
              msg.edit({ embeds: [frList] });
              await button.update({});
              break;
            case "right_frlist":
              if (page === maxpages) return button.update({});
              page++;
              x = x + 5;
              frList.fields = [];
              try {
