...
exports.run = async (client, message, args) => {
	...
	if (!guild.members.cache.get(message.author.id).voice.channel)
		return message.channel.send(
			textEmbed(":x: | You're not connected to a voice channel, please try again.")
		);
	...
	if (args.length > 1) {
		...
		for (arg of args) {
			var target = client.getUserFromMention(message.channel.guild, arg) || client.findRole(message.channel.guild, arg); 
			if (target) {
				try {
					await authorChannel.updateOverwrite(target, { CONNECT: true, VIEW_CHANNEL: true, SPEAK: true });
				} catch (err) {
					console.log(err);
				}
		...
		authorChannel
			.updateOverwrite(ab, { CONNECT: true, VIEW_CHANNEL: true, SPEAK: true })
			.then((test) => {
				message.reply(
					textEmbed(
